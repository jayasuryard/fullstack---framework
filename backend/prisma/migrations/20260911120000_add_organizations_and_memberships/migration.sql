...
-- PARTIAL UNIQUE INDEX (raw SQL — Prisma's schema language cannot express a
-- WHERE clause on an index). At most one *pending* invitation per (org, email):
-- accepted and revoked rows stay for audit and are excluded from the constraint,
-- so an email can be re-invited after being removed.
